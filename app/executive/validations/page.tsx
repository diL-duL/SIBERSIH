    ...
    return (
        <div className="pb-16 pt-8 min-h-screen bg-sibersih-bg flex flex-col max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
            ...
            <header className="mb-8 border-b border-sibersih-primary/10 pb-4 flex justify-between items-end">
                <div>
                    <h1 className="text-2xl font-light text-sibersih-primary tracking-tight">Validasi Pekerjaan</h1>
                    <p className="text-sm text-sibersih-primary/60 mt-1">Laporan yang membutuhkan persetujuan Anda</p>
                </div>
                <Link href="/executive/history" className="text-xs font-medium text-sibersih-primary/70 hover:text-sibersih-primary hover:underline">Riwayat Validasi</Link>
            </header>

            {butuhApproval.length === 0 ? (
                <div className="bg-white rounded-2xl border border-sibersih-primary/10 p-12 text-center flex flex-col items-center justify-center">
                    <Check className="w-12 h-12 text-forest-depths/30 mb-4" />
                    <h3 className="text-lg font-medium text-sibersih-primary mb-1">Semua Selesai!</h3>
                ...
                {butuhApproval.map((item) => (
                    <Card key={item.id} className="overflow-hidden flex flex-col justify-between border-sibersih-primary/10">
                        <div>
                            <CardHeader className="p-4 sm:p-5 flex flex-row justify-between items-start gap-2">
                                <div className="min-w-0 flex-1">
                                    <h3 className="font-medium text-base sm:text-lg text-sibersih-primary mb-1 line-clamp-1">
                                        {item.lokasi}
                                    </h3>
                                    <div className="flex flex-col gap-1 mt-1.5">
                                        <p className="text-xs sm:text-sm text-sibersih-primary/70 flex items-center gap-1.5 truncate">
                                            ...
                                            <span className="font-medium text-sibersih-primary truncate">{item.pelapor.nama}</span>
                                        </p>
                                        <p className="text-xs sm:text-sm text-sibersih-primary/70 flex items-center gap-1.5 truncate">
                                            <User size={13} className="text-forest-depths/70 shrink-0" />
                                            <span className="text-sibersih-primary/60 shrink-0">Petugas:</span>
                                            <span className="font-medium text-sibersih-primary truncate">{item.petugas?.nama || 'Petugas Kebersihan'}</span>
                                        </p>
                                    </div>
                                </div>
                                <Badge variant="warning" className="text-[11px] shrink-0 font-mono">Butuh Review</Badge>
                            </CardHeader>

                            <CardContent className="p-4 sm:p-5 pt-0 sm:pt-0 flex flex-col gap-3.5 sm:gap-4">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                    <div className="flex flex-col gap-1.5">
                                        <span className="text-[11px] font-mono text-sibersih-primary/60 uppercase tracking-wider flex items-center gap-1">
                                            <span className="w-1.5 h-1.5 rounded-full bg-slate-400" /> Sebelum (Laporan)
                                        </span>
                                        <div className="relative w-full h-44 sm:h-36 bg-warm-stone border border-sibersih-primary/10 rounded-xl overflow-hidden group">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <span className="text-[11px] font-mono text-forest-depths uppercase tracking-wider flex items-center gap-1">
                                            <span className="w-1.5 h-1.5 rounded-full bg-forest-depths" /> Sesudah (Hasil Petugas)
                                        </span>
                                        <div className="relative w-full h-44 sm:h-36 bg-warm-stone border border-sibersih-primary/10 rounded-xl overflow-hidden group">
                                ...
                                {item.deskripsiPetugas && (
                                    <div className="bg-warm-stone/70 p-3 rounded-xl border border-sibersih-primary/10">
                                        <span className="text-[10px] font-mono uppercase tracking-wider text-sibersih-primary/60 block mb-0.5">Catatan Petugas:</span>
