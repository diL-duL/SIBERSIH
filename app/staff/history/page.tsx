            ...
            <div className="max-w-3xl mx-auto w-full">
                ...
                <header className="mb-8">
                    ...
                    <p className="text-sm text-sibersih-primary/60 mt-1">
                        Daftar tugas kebersihan yang telah Anda kerjakan.
                    </p>
                </header>
                ...
                <div className="flex flex-col gap-4">
                    {completedTasks.length === 0 ? (
                        ...
                        completedTasks.map((tugas) => (
                            <Link href={`/staff/${tugas.id}`} key={tugas.id} className="bg-white rounded-xl shadow-sm border border-sibersih-primary/10 overflow-hidden flex flex-col sm:flex-row hover:border-sibersih-accent transition-colors">
                                ...
                                <div className="p-5 flex flex-col justify-center flex-1">
                                    <div className="flex items-start justify-between gap-4 mb-2">
                                        ...
                                        <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-md border shrink-0 ${tugas.status === 'SELESAI' ? 'bg-sibersih-accent/20 text-sibersih-primary border-sibersih-accent/30' : 'bg-orange-100 text-orange-700 border-orange-200'}`}>
                                            {tugas.status === 'SELESAI' ? <><CheckCircle size={14} /> Selesai</> : 'Menunggu Approval'}
                                        </span>
                                    </div>
